...
pub fn var_example() {
    ...
    println!(
        "Значение мутабельной (изменяемой по значению) переменной x = {}",
        x
    );
    x = 2;
    println!("Новое Значение x = {}", x);
    println!(
        "Не изменяемая (не мутабельная) переменная My Name = {}",
        my_name
    );
    println!(
        "Значение константы MAX_POINTS (не может быть <mut>) = {}",
        MAX_POINTS
    );
